#' MEMOIR.bias(): estimate the bias of a series of survival estimates generated  by
#'                MEMOIR.simplotter(), at a set of given sampling ages with a set of
#'                given true values.
#'
#' Returns the observed bias of the estimate at the given sampling ages. Output is
#' formatted as a dataframe, with ages as column one, proportional survival as column two,
#' and estimated bias as column three.
#'
...
MEMOIR.bias <- function(estimates, ages = c(5,10,15), proportions = c(0.75,0.5, 0.25)) {
    ...
    for (i in 1:nrow(pointsByIters)) {
        ...
        xBias <- matrix(data = NaN, nrow = 1, ncol = length(ages))
        for (j in 1:ncol(xErrors)) {
            samplePoint <- activeIter[abs(activeIter$ages - ages[j]*365)
                                      == min(abs(activeIter$ages - ages[j]*365)),]
            xBias[,j] <- samplePoint$propAlive[1] - proportions[j]
        }
        pointsByIters[i,] <- xBias
    }
    meanBias <- matrix(data = NaN, nrow = 1, ncol = ncol(squaredErrors))
    for (k in 1:ncol(meanSquaredErrors) ) {
        meanBias[,k] <- mean(pointsByIters[,k])
    }
    bias <- data.frame(ages = ages, propSurv = proportions, bias = as.vector(meanBias[1,]))
    return(bias)
}
